...
import { get_product_response, search_product_response } from 'src/state/actions/product_actions'
...
export function* search_products() {
  yield takeEvery(types.SEARCH_PRODUCTS_REQUEST, function* (data) {
    const { query } = data
    const response = yield call(products_api.search_products, query)
    yield put(search_product_response(response))
  })
}
